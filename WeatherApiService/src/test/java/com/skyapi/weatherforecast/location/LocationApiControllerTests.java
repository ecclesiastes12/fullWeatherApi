...
//		location3.setTrashed(true);
	...
	//unit test modified after LocationNotFoundException is handled by GlobalExceptionHandler.java. 
	//check LocationApiService1.java for the previous code
	...
	public void testGetShouldReturn404NotFound() throws Exception {
		...
		//create new LocationNotFoundException object
		LocationNotFoundException ex = new LocationNotFoundException(locationCode);
		Mockito.when(service.get(locationCode)).thenThrow(ex);
		
		//perform http request
		mockMvc.perform(get(requestURI))
			.andExpect(status().isNotFound()) //expects not found status code
			.andDo(print());
	}
...
//	}
	...
	//Unit test modified after LocationNotFoundException is handled by GlobalExceptionHander.java.
	//Check LocationApiService1.java for previous code
	//with DTO
	//test method for update location that return 404 not found response
	//with a given location code
	...
	public void testUpdateShouldReturn404NotFound() throws Exception {
		...
		mockMvc.perform(put(END_POINT_PATH).contentType("application/json").content(bodyContent))
			.andExpect(status().isNotFound()) //expected status code
			.andExpect(jsonPath("$.errors[0]", is(ex.getMessage()))) //expected message
			.andDo(print()); //print details of request body
	}
...
//	}
	...
	public void testDeleteShouldReturn404NotFound() throws Exception {
		...
		//throws exception if the location with the given code to delete is not found when the
		//delete method of the service class is invoked
		//code modified
		LocationNotFoundException ex = new LocationNotFoundException(code); //create new LocationNotFoundException object
		
		Mockito.doThrow(ex).when(service).delete(code);
		
		mockMvc.perform(delete(requestURI))
			.andExpect(status().isNotFound())
			.andExpect(jsonPath("$.errors[0]", is(ex.getMessage())))// expected error message
			.andDo(print());
	}
